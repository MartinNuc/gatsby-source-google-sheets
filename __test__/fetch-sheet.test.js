...
describe("cleaning rows from GSheets response", () => {
  ...
  it('guesses column types', function () {
    const rows = [{ number: "1", string: "something", null: null, boolean: "TRUE" }, { number: "2", string: "anything", null: null, boolean: "FALSE" }, { number: "3", string: "nothing", null: null, boolean: null }];
    const cleaned = cleanRows(rows);
    console.log(cleaned);
    expect(cleaned.map(row => row.number)).toEqual([1, 2, 3]);
